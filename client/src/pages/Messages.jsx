...
const Messages = () => {
  ...
  useEffect(() => {
    ...
    axiosInstance
      .get(`/api/v1/messages/${selectedConversation._id}`)
      .then((res) => {
  ...
  useEffect(() => {
    if (!user?._id) return;
    const onReconnect = () => socket.emit("user:join", user._id);
  ...
  return (
    <div
      className="flex bg-gray-50 dark:bg-gray-950/50"
      style={{ height: "calc(100vh - 56px)" }}
    >
      {/* Conversations sidebar */}
      <div className="w-72 flex-shrink-0 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700/60 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {conversations.length === 0 ? (
            <div className="p-4 text-sm text-gray-400 text-center">
              ...
              <Link
                to="/users"
                className="block mt-2 text-brand-500 hover:underline"
              >
                Find People
              </Link>
            </div>
